...
仅运行当前 Banner 自动抓取时，`python backend/capture.py` 不需要本地浏览器，可直接作为定时任务运行。只有 `--verify-dom`、`--legacy-dom-capture` 才需要 Edge/Chrome/Playwright Chromium；Wayback 默认通过 HTTP/API 恢复，显式 `--verify-dom` 时才需要浏览器。现有 `scripts/update_linux.sh` 无需改变入口。
